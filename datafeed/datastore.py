   ...
   '''
    def __init__(self, datadir, exchange, enable_rdb=True):
        ...
        if enable_rdb:
            ...
            self.meta = Meta(self._plaintale, exchange)
            self.tick = TickHistory(self._rstore, self.meta)
            self.depth = DepthHistory(self._rstore, self.meta)
            self.trade = TradeHistory(self._rstore, self.meta)
...
class RockStore(object):
    ...
    Rockstore keys are 48bits prefixed[1] with four pieces:
    - exchange: max 255 exchanges, 0x00 for unknown or default exchange
    - symbol: max 65535 symbols
    - table:  max 256 tables
    - offset: day offset to Year 1984, eg: 2000 would be transfer to 16

    +----------+----------+----------+----------+
    |  8bits   |  16bits  |  8bits   |  16bits  |
    +----------+----------+----------+----------+
    | exchange |  symbol  |  table   |  offset  |
    +----------+----------+----------+----------+

    The third byte is the leading byte of offseted[2] timestamp(millisecond)
    generated by simplefalke, so query for one-day rang data should no look more
    than 2 prefixes.
    ...
    TABLE_PREFIX = transform.int2bytes(0)

    def __init__(self, rdb, meta):
        assert self.TABLE_PREFIX and len(self.TABLE_PREFIX) == 1
    ...
    def open(cls, path):
        ...
        opts.prefix_extractor = FixedPrefixTransform(6)
    ...
    def query(self, symbol, timestamp):
        """Query rocksdb store with prefix"""
        prefix = self.prefix(symbol, timestamp)
        iter = self._rdb.iterkeys(prefix=prefix)
        iter.seek(prefix)
        return iter

    def query_values(self, symbol, timestamp):
        """Query rocksdb store with prefix"""
        prefix = self.prefix(symbol, timestamp)
    ...
    def query_items(self, symbol, timestamp):
        """Query rocksdb store with prefix"""
        prefix = self.prefix(symbol, timestamp)
    ...
    def range_prefix(self, symbol, start_time, stop_time):
    ...
    def prefix(self, symbol, timestamp):
        return self._prefix_key(symbol, timestamp)
    ...
    def _key(self, symbol, timestamp):
        return self._prefix_key(symbol, timestamp) + \
            transform.int2bytes(self._micro_timestamp(timestamp))
        
    def _prefix_key(self, symbol, timestamp):
        exchange = None
        symbol = symbol.encode('ascii')
        splited = symbol.split(':')
        if len(splited) == 2:
            exchange, symbol = splited
        #flakeid = simpleflake.simpleflake(timestamp) # 64bits
        #return self.TABLE_PREFIX + transform.int2bytes(flakeid, 8) + symbol
        exchange = self._meta.prefix_exchange(exchange)
        symbol = self._meta.prefix_symbol(symbol)
        self._prefix_timestamp(timestamp)
        return exchange + symbol + self.TABLE_PREFIX + \
            self._prefix_timestamp(timestamp)

    # EPOCH DAY: http://en.wikipedia.org/wiki/Nineteen_Eighty-Four
    _EPOCH_DAY = datetime.date(1984, 1, 1)
    def _prefix_timestamp(self, timestamp):
        day = datetime.date.fromtimestamp(timestamp)
        delta = day - self._EPOCH_DAY
        return transform.int2bytes(delta.days, 2)

    def _micro_timestamp(self, timestamp):
        if isinstance(timestamp, int):
            timestamp = timestamp + (time.time()%1) 
...
class Meta(PlainTableRockStore):
    ...
    TABLE_PREFIX = transform.int2bytes(0)
    DEFAULT_EXCHANGE = transform.int2bytes(0)

    KEY_EX_MAP = 'EXCHNAGES_MAP'
    KEY_SYMBOL_MAP = 'SYMBOLS_MAP'

    MAX_EX = 2**16 - 1

    def __init__(self, rdb, default_exchange):
        super(Meta, self).__init__(rdb)
    ...
    def prefix_exchange(self, exchange):
        ...
        lookup = str(exchange)
        if lookup in self._exchanges:
            return self._exchanges[lookup]
    ...
    def prefix_symbol(self, symbol):
        ...
        newv = transform.int2bytes(0, 2)
        if len(self._symbols) > 0:
            maxv = max(self._symbols.values())
    ...
    def prefix(self):
        ...
        return self.TABLE_PREFIX + transform.int2bytes(0)
...
class DepthHistory(RockStore):
    ...
    TABLE_PREFIX = transform.int2bytes(2)

class TradeHistory(RockStore):

    TABLE_PREFIX = transform.int2bytes(3)
